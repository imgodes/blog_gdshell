...
> Fiquei com dúvida em por que o `AttributeError: object has no attribute 'tab'` aparecia. Era porque o `addSuiteTab` chama `getUiComponent` imediatamente, antes do `initGui` ter rodado. A ordem de chamada no `registerExtenderCallbacks` é crítica.

> Fiquei com dúvida em por que o formulário ficava gigante. O `GridLayout` estica os componentes para preencher todo o espaço disponível. O `BorderLayout.NORTH` resolve isso ancorando o form no topo com tamanho natural.

> Fiquei com dúvida em como o botão chama o método `saveConfig`. No Jython, o Swing aceita `actionPerformed=self.saveConfig` direto no construtor do `JButton`, sem precisar criar um listener Java explícito.

> Fiquei com dúvida sobre o `global` no `saveConfig`. Sem ele, o Python criaria variáveis locais com o mesmo nome e as globais usadas pelo `sentToElastic` ficariam com os valores antigos.
> 
