...
Na nossa missão de criar circuitos digitais, usamos circuitos baseados em *switches* - e eles foram uma boa forma de introduzir o conceito. De qualquer forma, usar *switches* mecânicos não é uma boa ideia para computadores. Computadores precisam lidar com uma quantidade muito grande de entradas a todo momento, e acionar *switches* manualmente não é um bom *design*. Além disso, sistemas computacionais precisam conectar múltiplos circuitos lógicos juntos, onde a saída de um circuito será a entrada de outro.
...
BJTs possuem três terminais: **base**, **coletor** e **emissor**. Existem dois tipos de BJTs: NPN e PNP, que se diferenciam no modo de responder à aplicação de corrente no terminal base. Vamos focar no tipo NPN.
...
A ideia central é que em vez de apertar um *switch* físico, você aplica uma tensão em $V_{in}$. Essa tensão na base do transistor decide se ele conduz ou não:
...
A pergunta natural é: $V_{in}$ ainda precisa vir de algum lugar, então qual é a vantagem?
Com um switch mecânico, $V_{in}$ é um humano apertando um botão. Isso tem dois problemas sérios:
- **Velocidade.** Um transistor chaveia bilhões de vezes por segundo. Nenhum switch mecânico chega perto.
- **$V_{in}$ pode vir de outro circuito.** Esse é o ponto principal. O $V_{out}$ de um circuito pode ser o $V_{in}$ do próximo. Você encadeia circuitos, a saída de um vira entrada do outro, sem nenhuma intervenção humana. Um computador não tem um humano apertando switches internamente: tem trilhões de transistores onde a saída de um grupo controla o próximo, em cascata, na velocidade da eletricidade.
