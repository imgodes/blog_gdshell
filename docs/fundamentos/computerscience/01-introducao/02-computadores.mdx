...
Então você consegue comparar, a mão com um objeto e dizer o quão grande ou pequeno ele é por meio de uma analogia, e em outras palavras, é um tipo de representação analógica. Ela funciona por aproximação, comparação e variação contínua. Pequenas mudanças geram pequenas diferenças no resultado, sem saltos bruscos.
...
Discreto, nesse contexto, significa que os **valores são separados e distintos entre si, como degraus de uma escada. Não há valores intermediários entre eles, diferente de algo contínuo, que flui sem interrupção**. Lembre-se dessa definição pois será útil ao longo da nossa jornada.
...
É importante deixar algo claro neste ponto que **sinais elétricos não são números**.
...
Uma tensão em um fio não “vale” 0 ou 1 por natureza. Esses valores não existem na eletricidade. O que existe são apenas fenômenos físicos como corrente, tensão, carga e resistência. O que transforma esses fenômenos em algo útil é uma decisão humana de dar determinado significado aos fenômenos físicos.
...
Antes de falar sobre sistemas de numeração, é fundamental entender essa diferença. Os números 0 e 1 não surgem da eletricidade, e sim da necessidade de dar significado abstrato a estados físicos confiáveis. Para que isso funcione de forma consistente, é preciso um sistema que defina como esses símbolos representam valores. É exatamente esse o papel de um sistema de numeração.
