...
O computador pode ser visto como uma espécie de super calculadora. Claro que é muito simplista pensar assim, porém por baixo dos panos **tudo é número** (como diria Pitágoras). Tanto que não é difícil o ábaco ser mencionado na maioria dos livros de computação e em aulas de sistemas operacionais.
...
Euclides define número de forma conceitual. O número então, não é um objeto físico. Ele é uma abstração, uma ideia construída para possibilitar a representação de quantidades (1 carro, por exemplo). E *contar* é quando agrupamos mais de uma unidade de coisas iguais (2 carros).
...
Para Frege, números são objetos abstratos definidos por conceitos. Por exemplo, o número 3 não depende de quais objetos são contados, apenas do fato de que três objetos satisfazem uma mesma propriedade.
...
Até aqui falamos sobre o que é um número enquanto ideia. Vimos que números são abstrações, construções da mente humana usadas para representar quantidade, ordem e medida.

Um número, por si só, não ocupa espaço, não tem peso e não pode ser tocado. Para que ele seja usado por pessoas (e principalmente por máquinas) ele precisa ser representado de alguma forma.

Quando escrevemos o número 5, por exemplo, o que existe no papel ou na tela não é o número em si, mas um **símbolo** que concordamos em associar a essa ideia de 5 coisas. O símbolo poderia ser outro, desde que todos concordassem com o significado. O número continua o mesmo, mesmo que sua representação mude.
...
Depois de entender o que é um número, o próximo passo é entender como os números são escritos, organizados e interpretados. Antes de falar em binário, decimal, octal ou hexadecimal, precisamos compreender a ideia geral de que números são ideias; sistemas de numeração são as linguagens que usamos para expressar essas ideias!